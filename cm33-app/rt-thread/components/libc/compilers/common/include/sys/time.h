...
/* When the toolchain's sys/types.h includes sys/select.h (BSD visible), it
 * pulls in RT-Thread's sys/select.h -> sys/time.h -> time.h before reaching
 * the clock_t/time_t typedefs later in the same file. Define the missing types
 * early so the toolchain's time.h can use them. */

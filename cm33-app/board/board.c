...
#include "board.h"
#include "../applications/shared_data.h"
...
static void cm33_boot_marker(const char *text, uint32_t version)
{
    volatile shared_data_t *shm = SHARED_DATA_ADDR;
    uint32_t i;

    shm->data_len = 0;
    for (i = 0; text[i] != '\0' && i < sizeof(shm->payload) - 1; i++)
    {
        shm->payload[i] = text[i];
    }
    shm->payload[i] = '\0';
    shm->data_len = i;
    shm->cm33_heartbeat = version;
...
void cy_bsp_all_init(void)
{
    ...
    cm33_boot_marker("cm33 board init", 0x33000001UL);

...
static void cy_sdio_dump_xfer_state(struct cy_pse_sdio *sdio, struct rt_mmcsd_cmd *cmd,
                                    struct rt_mmcsd_data *data, cy_rslt_t result)
{
    SDHC_Type *base = sdio->hw_desc->hw_base;

    LOG_E("xfer diag: result=0x%08x cmd=%d arg=0x%08x rw=%c len=%u blks=%u blksize=%u",
          (unsigned int)result,
          cmd->cmd_code,
          cmd->arg,
          (data && (data->flags & DATA_DIR_WRITE)) ? 'w' : 'r',
          data ? (unsigned int)(data->blks * data->blksize) : 0U,
          data ? (unsigned int)data->blks : 0U,
          data ? (unsigned int)data->blksize : 0U);

    LOG_E("xfer diag: NormIntSt=0x%04x ErrIntSt=0x%04x NormIntEn=0x%04x NormIntMask=0x%04x ErrIntEn=0x%04x",
          (unsigned int)SDHC_CORE_NORMAL_INT_STAT_R(base),
          (unsigned int)SDHC_CORE_ERROR_INT_STAT_R(base),
          (unsigned int)SDHC_CORE_NORMAL_INT_STAT_EN_R(base),
          (unsigned int)Cy_SD_Host_GetNormalInterruptMask(base),
          (unsigned int)SDHC_CORE_ERROR_INT_STAT_EN_R(base));

    LOG_E("xfer diag: PresentState=0x%08x BlkCnt=0x%04x XferMode=0x%04x ADMA_Err=0x%02x ADMA_SA=0x%08x",
          (unsigned int)SDHC_CORE_PSTATE_REG(base),
          (unsigned int)SDHC_CORE_BLOCKCOUNT_R(base),
          (unsigned int)SDHC_CORE_XFER_MODE_R(base),
          (unsigned int)SDHC_CORE_ADMA_ERR_STAT_R(base),
          (unsigned int)SDHC_CORE_ADMA_SA_LOW_R(base));
}
...
static rt_err_t cy_sdio_send_command(struct cy_pse_sdio *sdio, struct rt_mmcsd_cmd *cmd)
{
    ...
    if ((err == RT_EOK) && data)
    {
        ...
        if (CY_RSLT_SUCCESS != result)
        {
            ...
            cy_sdio_dump_xfer_state(sdio, cmd, data, result);
            err = -RT_ERROR;
            goto exit_hw;
        }
...
{
    struct cy_pse_sdio *sdio = host->private_data;
    uint32_t interrupt_enable_status;
    uint32_t interrupt_mask;

    RT_ASSERT(sdio != RT_NULL);

    interrupt_enable_status = Cy_SD_Host_GetNormalInterruptEnable(sdio->hw_desc->hw_base);

    if (en)
    {
        interrupt_mask = Cy_SD_Host_GetNormalInterruptMask(sdio->hw_desc->hw_base);
        Cy_SD_Host_SetNormalInterruptMask(sdio->hw_desc->hw_base,
                                          interrupt_mask | CY_SD_HOST_CARD_INTERRUPT);
        interrupt_enable_status |= CY_SD_HOST_CARD_INTERRUPT;
    }
    else
    {
        interrupt_enable_status &= (uint32_t) ~CY_SD_HOST_CARD_INTERRUPT;
    }

    Cy_SD_Host_SetNormalInterruptEnable(sdio->hw_desc->hw_base, interrupt_enable_status);
}

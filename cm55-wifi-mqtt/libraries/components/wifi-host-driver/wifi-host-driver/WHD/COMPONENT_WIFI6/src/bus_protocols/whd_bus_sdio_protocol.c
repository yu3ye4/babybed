...
whd_result_t whd_bus_sdio_init(whd_driver_t whd_driver)
{
    ...
    WPRINT_WHD_ERROR( ("[sdio-init] wait F2 ready\n") );
    loop_count = 0;
    while ( ( (result = whd_bus_read_register_value(whd_driver, BUS_FUNCTION, SDIOD_CCCR_IORDY, (uint8_t)1,
                                                    &byte_data) ) == WHD_SUCCESS ) &&
    ...
    if (result != WHD_SUCCESS)
    {
        WPRINT_WHD_ERROR( ("[sdio-init] F2 ready read failed: 0x%08" PRIx32 " last_iordy:0x%02x loop:%" PRIu32 "\n",
                           (uint32_t)result, byte_data, loop_count) );
        return result;
    }
    WPRINT_WHD_ERROR( ("[sdio-init] F2 ready iordy:0x%02x loop:%" PRIu32 "\n", byte_data, loop_count) );
    ...
    WPRINT_WHD_ERROR( ("[sdio-init] chip specific init\n") );
    ...
    CHECK_RETURN(result);
    WPRINT_WHD_ERROR( ("[sdio-init] ensure bus up\n") );
    ...
    CHECK_RETURN(result);
    WPRINT_WHD_ERROR( ("[sdio-init] enable irq\n") );
...
#define TRX_HDR_START_ADDR 0x7fd4c /* TRX header start address */
    ...
    WPRINT_WHD_ERROR( ("[sdio-dl] start resource:%d blocks:%" PRIu32 " size:%" PRIu32 " addr:0x%08" PRIx32 "\n",
                       (int)resource, blocks_count, image_size, address) );

    for (i = 0; i < blocks_count && image_size > 0; i++)
    {
        WPRINT_WHD_ERROR( ("[sdio-dl] block:%" PRIu32 "/%" PRIu32 " get\n", i, blocks_count) );
        CHECK_RETURN(whd_get_resource_block(whd_driver, resource, i, (const uint8_t **)&image, &size_out) );
        ...
        WPRINT_WHD_ERROR( ("[sdio-dl] block:%" PRIu32 "/%" PRIu32 " write addr:0x%08" PRIx32 " len:%" PRIu32 "\n",
                           i, blocks_count, address, size_out) );
        ...
        WPRINT_WHD_ERROR( ("[sdio-dl] block:%" PRIu32 "/%" PRIu32 " done\n", i, blocks_count) );
        address += size_out;
    }
#ifdef WPRINT_ENABLE_WHD_DEBUG

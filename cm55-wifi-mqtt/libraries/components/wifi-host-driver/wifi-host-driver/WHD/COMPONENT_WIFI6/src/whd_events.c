...
static whd_result_t whd_management_send_event_msgs(whd_interface_t ifp, whd_interface_t prim_ifp,
                                                   whd_event_info_t *event_info)
{
    whd_buffer_t buffer;
    uint8_t *event_mask;
    uint16_t i;
    uint16_t j;
    whd_result_t res = WHD_PENDING;
    ...
    for (uint8_t attempt = 0; attempt < 3; attempt++)
    {
        ...
        WPRINT_WHD_ERROR( ("%s: send event_msgs(iovar) failed, res=0x%08lx attempt=%u\n",
                           __func__, (unsigned long)res, (unsigned int)(attempt + 1)) );
                                              ...
                                              void *handler_user_data, uint16_t *event_index)
{
    ...
    res = whd_management_send_event_msgs(ifp, prim_ifp, event_info);
    if (res != WHD_SUCCESS)
    {
        goto set_event_handler_exit;
    }

    ...
    while (thread_info->thread_quit_flag != WHD_TRUE)
    {
                ...
                (whd_bus_use_status_report_scheme(whd_driver) ) )
        {
            ...
            else
            {
                ...
                {
                    /* The SDIO card interrupt/status can be missed on this
                     * RT-Thread port. Try one bounded receive pass so pending
                     * IOCTL/event frames are not left until the next command.
                     */
                    do
                    {
                        rx_status = whd_thread_receive_one_packet(whd_driver);

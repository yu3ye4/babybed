...
#define WHD_IOCTL_TIMEOUT_MS         (10000)    /** Need to give enough time for coming out of Deep sleep (was 400) */
#define WHD_IOCTL_WAIT_SLICE_MS      (100)
                                ...
                                whd_buffer_t *response_buffer_hnd)
{
    ...
    /* Wait till response has been received. Keep nudging the WHD thread because
     * the RT-Thread SDIO IRQ path can miss a card interrupt in safe mode.
     */
    do
    {
        uint32_t wait_ms = MIN_OF(WHD_IOCTL_WAIT_SLICE_MS, WHD_IOCTL_TIMEOUT_MS - waited_ms);
        retval = cy_rtos_get_semaphore(&cdc_bdc_info->ioctl_sleep, wait_ms, WHD_FALSE);
        if (retval == WHD_SUCCESS)
        {
            break;
        }
    ...
    } while (waited_ms < WHD_IOCTL_TIMEOUT_MS);

...
//--------------------------------------------------------------------------------------------------
// _mtb_hal_sdxx_xfer_done_in_hw
//--------------------------------------------------------------------------------------------------
static bool _mtb_hal_sdxx_xfer_done_in_hw(_mtb_hal_sdxx_t* sdxx)
{
    uint32_t normal_status = Cy_SD_Host_GetNormalInterruptStatus(sdxx->base);
    uint32_t error_status = Cy_SD_Host_GetErrorInterruptStatus(sdxx->base);
    uint32_t present_state = Cy_SD_Host_GetPresentState(sdxx->base);
    uint32_t data_busy = CY_SD_HOST_CMD_CMD_INHIBIT_DAT | CY_SD_HOST_DAT_LINE_ACTIVE;

    return ((normal_status & CY_SD_HOST_ERR_INTERRUPT) == 0UL) &&
           (error_status == 0UL) &&
           (Cy_SD_Host_GetAdmaErrorStatus(sdxx->base) == 0UL) &&
           (SDHC_CORE_BLOCKCOUNT_R(sdxx->base) == 0UL) &&
           ((present_state & data_busy) == 0UL);
}
...
static cy_rslt_t _mtb_hal_sdxx_waitfor_transfer_complete(_mtb_hal_sdxx_t* sdxx)
{
    ...
    if ((!in_isr) && _mtb_hal_sdxx_is_smfr_ready_for_get(sdxx))
    {
        ...
        else if (_mtb_hal_sdxx_xfer_done_in_hw(sdxx))
        {

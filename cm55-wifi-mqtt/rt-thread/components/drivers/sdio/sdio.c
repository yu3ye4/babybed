                                ...
                                rt_uint32_t          *cmd5_resp)
{
    ...
    for (i = 100; i; i--)
    {
        ...
        else
        {
            ...
            /* SDIO R4 response uses bits [31:28] for function count.
             * Cards with 1-7 functions legitimately have bit31 = 0.
             * CYW55500 (1 function) never sets bit31. After 10
             * successful responses (100ms), the card is clearly
             * alive and ready — proceed without CARD_BUSY. */
            if (i <= 90 && cmd.resp[0] != 0)
                break;
        }
...
{
    rt_int32_t err = 0;
    rt_int32_t i, function_num, probe_mode;
    rt_uint32_t  cmd5_resp;
    struct rt_mmcsd_card *card;
    int bypass_init;

    /* CYW55500 workaround: if ocr == 0, this is a non-standard card whose
     * probe OCR had bits in the reserved low 7 bits. The hardware already
     * powers the bus at the correct voltage. Standard CMD5 voltage
     * negotiation would put the card into a broken state.
     *
     * In bypass mode we also skip CMD3/CMD7/CCCR/CIS/function-init because
     * the chip doesn't respond to CMD3/CMD7, and the WHD driver's
     * whd_bus_sdio_init() handles all configuration via its own CMD52/CMD53
     * sequence. SDIO CMD52/CMD53 don't need RCA or card selection. */
    bypass_init = (ocr == 0);

    if (bypass_init)
        err = sdio_io_send_op_cond(host, 0, &cmd5_resp);
    else
    ...
    if (!controller_is_spi(host))
    {
        ...
        {
            card->rca = 0x5555;
            LOG_W("non-standard SDIO card, using synthetic RCA 0x%04x", card->rca);
        }
        else
        {
    ...
    if (bypass_init)
    {
        /* Non-standard card (CYW55500): skip CCCR/CIS/function-init.
         * The WHD driver's whd_bus_sdio_init() handles all card
         * configuration via its own CMD52/CMD53 sequence after probe. */
        LOG_W("bypassing standard SDIO init, WHD driver will configure card");

        /* Minimal CIS data so WHD driver's sdio_match_card() matches */
        card->cis.manufacturer = 0x04B4;
        card->cis.func0_blk_size = 64;
        card->cis.max_tran_speed = 25000000;

        /* Create minimal function 1 for WHD driver matching */
        card->sdio_function_num = 1;
        card->sdio_function[1] = rt_malloc(sizeof(struct rt_sdio_function));
        if (!card->sdio_function[1])
        {
            err = -RT_ENOMEM;
            goto err2;
        }
        rt_memset(card->sdio_function[1], 0, sizeof(struct rt_sdio_function));
        card->sdio_function[1]->card = card;
        card->sdio_function[1]->num = 1;
        card->sdio_function[1]->product = 48446;

        /* The WHD driver will switch the card to 4-bit bus width via CMD52
         * (CCCR BICTRL). Switch the host side too, otherwise CMD53 block-mode
         * writes will fail because the host still drives only DAT[0] while
         * the card expects data on DAT[0:3]. */
...
rt_int32_t init_sdio(struct rt_mmcsd_host *host, rt_uint32_t ocr)
{
    ...
    /* Detect non-standard SDIO cards like CYW55500: bits set in the
     * reserved low 7 bits means the OCR doesn't follow the standard. */
    skip_voltage = (ocr & 0x7F) ? 1 : 0;
    ...
    if (ocr & VDD_165_195)
    {
        LOG_D("Card supports 1.8V low voltage, keeping it");
    }

    /* For 1.8V-only SDIO cards like CYW55500 that don't report standard 3.3V bits,
     * force-add VDD_165_195 so voltage negotiation picks 1.8V */
    if (!(ocr & (VDD_27_28 | VDD_28_29 | VDD_29_30 | VDD_30_31 |
                 VDD_31_32 | VDD_32_33 | VDD_33_34 | VDD_34_35 | VDD_35_36)))
    {
        ocr |= VDD_165_195;
    }
    ...
    /* For non-standard cards (CYW55500), skip CMD5 voltage negotiation.
     * The hardware already powers the bus at the correct voltage.
     * Sending CMD5 with a voltage arg puts the card into a broken state. */
    err = sdio_init_card(host, skip_voltage ? 0 : current_ocr);
    if (err)
        goto remove_card;

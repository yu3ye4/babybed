...
#ifdef MQTT_DEBUG
#ifndef APP_DEBUG_PAHO_VERBOSE
#define APP_DEBUG_PAHO_VERBOSE 0
#endif
#if APP_DEBUG_PAHO_VERBOSE
 ...
 */
static int mqtt_resolve_uri(MQTTClient *c, struct addrinfo **res, rt_bool_t *res_is_static)
{
    int rc = 0;
    ...
    uri_len = strlen(uri);
    if (res_is_static != RT_NULL)
    {
        *res_is_static = RT_FALSE;
    }
    ...
    /* get host addr ok. */
    {
        ...
        if (inet_addr(host_addr_new) != INADDR_NONE)
        {
            static struct addrinfo numeric_ai;
            static struct sockaddr_in numeric_addr;

            rt_memset(&numeric_ai, 0, sizeof(numeric_ai));
            rt_memset(&numeric_addr, 0, sizeof(numeric_addr));

            numeric_addr.sin_family = AF_INET;
            numeric_addr.sin_port = htons((uint16_t)atoi(port_str));
            numeric_addr.sin_addr.s_addr = inet_addr(host_addr_new);

            numeric_ai.ai_family = AF_INET;
            numeric_ai.ai_socktype = SOCK_STREAM;
            numeric_ai.ai_protocol = 0;
            numeric_ai.ai_addr = (struct sockaddr *)&numeric_addr;
            numeric_ai.ai_addrlen = sizeof(numeric_addr);

            *res = &numeric_ai;
            if (res_is_static != RT_NULL)
            {
                *res_is_static = RT_TRUE;
            }
            goto _exit;
        }
...
static int net_connect(MQTTClient *c)
{
    ...
    rt_bool_t addr_res_static = RT_FALSE;
    ...
    rc = mqtt_resolve_uri(c, &addr_res, &addr_res_static);
    ...
    {
        LOG_E("connect err! errno:%d", errno);
        closesocket(c->sock);
        c->sock = -1;

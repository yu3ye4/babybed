                                ...
                                rt_uint32_t          *cmd5_resp)
{
    ...
    for (i = 100; i; i--)
    {
        ...
        else
        {
            ...
            /* CYW55500 reports a non-standard R4 response during CMD5.
             * A non-zero response after repeated probes means the card is
             * alive, even if CARD_BUSY is not set in the standard position.
             */
...
static rt_int32_t sdio_init_card(struct rt_mmcsd_host *host, rt_uint32_t ocr)
{
    ...
    /* CYW55500 workaround: if ocr == 0, skip standard SDIO voltage
     * negotiation and card selection. WHD configures the chip later with
     * its own CMD52/CMD53 sequence.
     */
    bypass_init = (ocr == 0);
